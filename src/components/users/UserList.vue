...
export default {
  ...
  // Components arent replaced by the Composition API

  emits: ['list-projects'],

  props: ['users'],
  // Props arent replaced by the Composition API
  ...
  setup(props) {
    ...
    const availableUsers = computed(function () {
      ...
      if (activeSearchTerm.value) {
        ...
        ); // this.users => props.users because user in props
      } else if (props.users) {
        users = props.users;
      }
      return users;
    ...
    const displayedUsers = computed(function () {
      ...
      return availableUsers.value.slice().sort((u1, u2) => {
        if (sorting.value === 'arc' && u1.fullName > u2.fullName) {
          return 1;
        } else if (sorting.value === 'arc') {
          return -1;
